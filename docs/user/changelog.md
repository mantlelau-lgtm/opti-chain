...
### Added
- 审批流：按订单类型定义审批组（会签/或签），草稿单「提交审批」→ 组员在「审批工作台」审批
- 审批工作台：待我审批/我已处理/我提交的三个列表，点开弹窗看单据明细+审批进度+通过/驳回（含意见）
- 审批完成自动生效：采购单→已审批；销售单→锁库+信用；驳回退回草稿
- 权限：approval:view（工作台）、approval:manage（审批组管理）

### Changed
- 采购单列表移除状态下拉框（审批改由审批流驱动），保留「取消」按钮
...
- 数据源配置页显示「当前数据源」，与当前同类型的数据源标记「当前类型」并禁用迁移

### Fixed
- 迁移幂等：重跑前自动清空目标库本应用的表，不再主键冲突
- 迁移外键：复制包在单事务内并关闭外键约束，兼容源库孤儿明细数据
