...
func (h *PlanningHandler) DemandList(c *gin.Context) {
	list, total, err := h.svc.ListDemands(c.Request.Context(), tenantOf(c), parsePage(c))
	if mapErr(c, err) {
		return
	}
...
func (h *PlanningHandler) DemandGet(c *gin.Context) {
	m, err := h.svc.GetDemand(c.Request.Context(), tenantOf(c), idParam(c))
	if mapErr(c, err) {
		return
	}
	response.OK(c, m)
...
func (h *PlanningHandler) DemandCreate(c *gin.Context) {
	...
	if err := h.svc.CreateDemand(c.Request.Context(), tenantOf(c), d); mapErr(c, err) {
		return
	}
	response.OK(c, d)
}
...
func (h *PlanningHandler) DemandUpdate(c *gin.Context) {
	...
	if err := h.svc.UpdateDemand(c.Request.Context(), tenantOf(c), idParam(c), d); mapErr(c, err) {
		return
	}
	response.OK(c, d)
}
...
func (h *PlanningHandler) DemandDelete(c *gin.Context) {
	if mapErr(c, h.svc.DeleteDemand(c.Request.Context(), tenantOf(c), idParam(c))) {
		return
	}
...
func (h *PlanningHandler) MrpList(c *gin.Context) {
	list, total, err := h.svc.ListMrp(c.Request.Context(), tenantOf(c), parsePage(c))
	if mapErr(c, err) {
		return
	}
	response.OKPage(c, total, list)
...
func (h *PlanningHandler) MrpGet(c *gin.Context) {
	m, err := h.svc.GetMrp(c.Request.Context(), tenantOf(c), idParam(c))
	if mapErr(c, err) {
		return
	}
	response.OK(c, m)
...
func (h *PlanningHandler) MrpDelete(c *gin.Context) {
	if mapErr(c, h.svc.DeleteMrp(c.Request.Context(), tenantOf(c), idParam(c))) {
		return
	}
	response.OK(c, gin.H{"id": idParam(c)})
}
...
func (h *PlanningHandler) ComputeMRP(c *gin.Context) {
	...
	results, err := h.svc.ComputeMRP(c.Request.Context(), tenantOf(c), batch)
	if mapErr(c, err) {
		return
	}
...
func (h *PlanningHandler) MrpConvert(c *gin.Context) {
	...
	po, err := h.svc.ConvertMRP(c.Request.Context(), tenantOf(c), idParam(c), body.PONumber)
	if mapErr(c, err) {
		return
	}
	response.OK(c, po)
